...
use axum::Router;
use crucible_server::{
    router, try_flush_once, AppState, FlushOutcome, JsonCuratedBank, MultiBank, ServerConfig,
    StaticMathBank,
};
...
fn spawn_flusher(state: Arc<AppState>, dir: PathBuf, flush_secs: u64) {
    tokio::spawn(async move {
        ...
        loop {
            ...
            match try_flush_once(&state, &dir).await {
                FlushOutcome::NothingToDo => {}
                FlushOutcome::Wrote { patterns, target } => {
                    eprintln!(
                        "crucible-serve: flushed {patterns} patterns to {}",
                        target.display()
                    );
                }
                FlushOutcome::Failed {
                    ...
                    eprintln!(
                        "crucible-serve: flush failed ({}): {error} — re-queueing {requeued} tuples for next flush",
                        target.display()
                    );
                }
            }
